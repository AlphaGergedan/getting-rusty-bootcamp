...
Dz.init = function () {
    ...
    this.slides = Array.prototype.slice.call(dzQueryAll("body > section"));
    this.progressBar = dzQuery("#progress-bar");
...
Dz.toggleContent = function () {
    ...
    var s = dzQuery("section[aria-selected]");
    if (s) {
        var video = s.dzQuery("video");
        if (video) {
...
Dz.onhashchange = function () {
    ...
    if (cursor.length == 2) {
        ...
        if (newstep > Dz.slides[newidx - 1].dzQueryAll('.incremental > *').length) {
            newstep = 0;
            newidx++;
        }
    }
...
Dz.setSlide = function (aIdx) {
    this.idx = aIdx;
    var old = dzQuery("section[aria-selected]");
    var next = dzQuery("section:nth-of-type(" + this.idx + ")");
    if (old) {
        old.removeAttribute("aria-selected");
        var video = old.dzQuery("video");
        if (video) {
            video.pause();
        }
    }
...
Dz.setIncremental = function (aStep) {
    this.step = aStep;
    var old = this.slides[this.idx - 1].dzQuery('.incremental > *[aria-selected]');
    if (old) {
        old.removeAttribute('aria-selected');
    }
    var incrementals = dzQueryAll('.incremental');
    if (this.step <= 0) {
    ...
    var next = this.slides[this.idx - 1].dzQueryAll('.incremental > *')[this.step - 1];
    if (next) {
        next.setAttribute('aria-selected', true);
...
var dzQuery = (HTMLElement.prototype.dzQuery = function (aQuery) {
    return this.querySelector(aQuery);
}).bind(document);

var dzQueryAll = (HTMLElement.prototype.dzQueryAll = function (aQuery) {
